...
custom_pink_theme = Theme(
    background_color=BG_PINK,  # Pink background
    title_background_color=BG_PINK,  # Hot pink for the title background
    title_font_color=(255, 255, 255),  # White title text
    title_font_size=50,  # Title font size
    widget_font=pygame_menu.font.FONT_8BIT,  # Custom font for widgets
    widget_font_color=(50, 50, 50),  # Dark gray for widget text
    widget_font_size=50,  # Widget font size
    widget_background_color=(255, 228, 225),  # Light pink for widgets
    widget_selection_effect=pygame_menu.widgets.HighlightSelection(),  # Widget hover effect
    widget_padding=(20, 10),  # Add gap between the buttons (vertical padding)
    widget_border_color=(255, 228, 225),  # Remove border by matching background color
    widget_border_width=0,  # Make the border width zero to avoid any borders
)
    ...
    for rowIndex, row in enumerate(WORLD):
        for colIndex, cell in enumerate(row):
            ...
            if cell == 1:
                ...
                cherry_image = pygame.image.load("cherry.png")  # Load the cherry image
                ...
                continue  # Skip the rest of the code for this cell, since we're
            pygame.draw.rect(
                DISPLAYSURF,
                currentColor,
